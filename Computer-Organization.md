...
> 一个个电路组合,制作了一个完整功能的CPU

###### 单指令周期处理器

一条CPU指令有Fetch  ---> Decode ---> Execute三个步骤组成,至少花费一个时钟周期

一个时钟周期可以执行一条指令, CPI也是1 , 这种叫做单指令周期处理器

时钟周期是固定的但是电路复杂程度不同,所以一条指令执行时间不同,随着门电路的增加,由于门延迟的存在,位数多,计算复杂的指令需要执行时间会更长

![指令执行](https://github.com/test-wsl/Computer-Organization/blob/master/img/%E6%8C%87%E4%BB%A4%E6%89%A7%E8%A1%8C.jpeg?raw=true)

> 不同指令的执行时间不同,但是要让所有的指令在一个时钟周期内完成,那么只好让时钟周期和执行时间最长的指令一样.

> 无论是一条用不到ALU的无条件跳转指令,还是一条计算起来特别复杂的浮点数乘法运算,都需要等满一个时钟周期,这种情况下CPI能保持1,但是时钟频率无法太高

![前一条指令写入,后一条指令读取](https://github.com/test-wsl/Computer-Organization/blob/master/img/%E5%89%8D%E4%B8%80%E6%9D%A1%E6%8C%87%E4%BB%A4%E7%9A%84%E5%86%99%E5%85%A5,%E5%90%8E%E4%B8%80%E6%9D%A1%E6%8C%87%E4%BB%A4%E7%9A%84%E8%AF%BB%E5%8F%96%E4%B9%8B%E5%89%8D.jpeg?raw=true)

###### 现代处理器的流水线设计

> 指令执行的过程会拆分为"取指令 ,译码, 执行"三大步骤 , 执行过程还包含从寄存器或者内存中读取数据 ,通过ALU运算,把结果写到寄存器或者内存中\

![流水线执行](https://github.com/test-wsl/Computer-Organization/blob/master/img/%E6%B5%81%E6%B0%B4%E7%BA%BF%E6%89%A7%E8%A1%8C.jpeg?raw=true)

* 指令流水线 ,每一个独立的步骤为流水线阶段或者流水线级
* 将指令拆分为"取指令 -- 指令译码 -- ALU计算 -- 内存访问 -- 数据写回" 会变为一个五级流水线 ,在同一个时钟周期内,可以同时运行五条指令的不同阶段 , 虽然一条指令的时钟周期变成了5 ,但是可以把CPU主频提高, 不再需要确保最复杂的那条指令在时钟周期中完成,只要保障一个最复杂的流水线级的操作,在一个时钟周期内完成就可以

> 不能减少单指令延时的性能指标, 但是同时执行多条指令的不同阶段,提升CPU的吞吐率

###### 超长流水线的性能瓶颈

* 增加流水线深度,会有性能成本

![单指令流水线执行](https://github.com/test-wsl/Computer-Organization/blob/master/img/%E5%8D%95%E6%8C%91%E6%8C%87%E4%BB%A4%E7%9A%84%E6%B5%81%E6%B0%B4%E7%BA%BF.jpeg?raw=true)

* 同步时钟周期,不再是指令级别的,二十流水线阶段级别的,每一级流水线对应的输出,都要放到流水线寄存器,然后在下一个时钟周期,交给下一个流水线级去处理,每增加一级写入到流水线寄存器的操作

###### 总结

* 为了不浪费CPU性能,通过指令的执行过程,切为一个一个流水线级,提升CPU'吞吐率,本身的CPU设计,是由一个个独立的组合逻辑电路串接起来,适合采用这种分工的工作方式
* 一味的增加流水线深度,不能无限制的提高性能,同样,指令的执行不再是一条一条的顺序的执行,二十在上一条执行的时候,下一条已经启动.
* 推荐
  * 《深入理解计算机系统》的 4.4 章节
  * 《计算机组成与设计 硬件 / 软件接口》的 4.5 章节
* 思考
  * 一个CPU时钟周期,可以认为是完成一条简单指令的时间,正确吗?如何理解
    * 
...
流水线技术通过将一条指令操作切分为更细的步骤,可以避免CPU浪费,每一个细分的流水线步骤都很简单,使我们的单个时钟周期的时间变得更短

###### 主频战争带来的超长流水线

* 不能通过CPU的主频来衡量CPU乃至计算机整机的性能,不同的CPU实际体系架构和实现都不同,实际的性能可能差别很大,最好的衡量结果是通过SPEC的跑分程序

Intel为了到达10GHz,在NetBurst架构上使用了超长的流水线,增加流水线深度,在同主频下,降低了CPU性能,因为一个Pipeline Stage需要一个时钟周期,把任务分为31阶段,就需要31个周期才能完成一个任务,每个Stage都需要对应的Pipeline寄存器的开销,更深的流水线性能会差一些

![流水线时间比较](https://github.com/test-wsl/Computer-Organization/blob/master/img/%E6%B5%81%E6%B0%B4%E7%BA%BF%E6%97%B6%E9%97%B4%E6%AF%94%E8%BE%83.jpeg?raw=true)

> 流水线技术不能缩短单条指令的响应时间,但可以增加在多条指令时的吞吐率,不同的指令实际需要的时间不同.

每一条指令从开始到结束拿到结果的时间并没有变化,,及响应时间没有变化,但是同样时间内,完成的指令数增加,吞吐率上升.

###### 新的挑战:冒险和分支预测

>1. 功耗,提升了流水线深度必须和提升CPI主频同时进行,只有提升时钟周期,CPU指令的响应时间才能保持原来相同的性能
>
>   流水线深度的增加,使我们需要的电路数量变多
>
>   主频提升和晶体管数量的增加,使得CPU功耗变大
>
>2. 流水线技术带来的性能提升,是一个理想状态,在实际程序执行中,不一定做得到
...
* 先执行无依赖关系的代码,然后在执行其他代码
* 几十条没有依赖关系的指令不好找到,会导致超长流水线的执行效率而降低

###### 总结

* 一个合理的流水线深度,会提升我们CPU执行计算机指令的吞吐率,一般使用IPC衡量
* IPC 就是CPI的倒数  IPC=3 那么CPI=0.33
* 需要引入乱序执行, 分支预测等技术
* 推荐阅读
